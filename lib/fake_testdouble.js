  ...
  module.exports = function(__userDoesPretendInvocationHere__, config) {
    ...
    if (last) {
      if (callsStore.wasInvoked(last.testDouble, last.args, config)) {
        ...
        //console.log("error b/c we called it wrong or didn't call it");

        err = new Error(unsatisfiedErrorMessage(last.testDouble, last.args, config));
        err.testDouble = last.testDouble;
        err.calls = callsStore["for"](last.testDouble);
        err.expectedArgs = last.args;
        err.config = config;
        throw err;
      }
    } else {
      //console.log("error b/c we didn't use verify() right");
      err = new Error("No test double invocation detected for `verify()`.\n\n  Usage:\n    verify(myTestDouble('foo'))");
      err.testDouble = null
      err.expectedArgs = null
      err.config = config;
      throw err;
    }
  };
